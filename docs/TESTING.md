...
|------|-------------|--------|
| Backend (`backend/`) | `pytest` (installed, in `requirements.txt`) | Two modules have real automated coverage: [`test_repo_fetcher.py`](../backend/tests/test_repo_fetcher.py) (72 tests) and [`test_embedding_url.py`](../backend/tests/test_embedding_url.py) (24 tests) — **all 96 passing**. `test_dpga_real_positives.py` and `test_gitlab_provider.py` remain manual, live-network scripts (excluded from collection via `conftest.py`'s `collect_ignore`) — useful as smoke tests, not part of the automated suite. `test_per_sdg_thresholds.py` (20 tests, offline/mocked) covers the per-SDG threshold feature from `EVAL_DPGA_150_RESULTS.md` §3. `aurora_api.py`, `summariser.py`, `sdg_constants.py`, and `app.py` (§3–§6 below) still have no dedicated test files (though `test_per_sdg_thresholds.py` exercises `sdg_constants.PER_SDG_THRESHOLDS`, `passes_threshold`, and `app._st_pred_passes`). |
...
Run the backend suite with `cd backend && pytest tests/` (the manual scripts are auto-excluded). The manual audit of flagged LLM summaries (recommendation #6 in `docs/EVAL_DPGA_150_RESULTS.md`) is tooled by `backend/tests/audit_summary_flags.py`, which turns a cached eval report's `report["flags"]` (low word-overlap, SDG leaks, numeric claims not in source) into a Markdown review worksheet. Setting up runners for `models/` and the frontend (`Jest` + `React Testing Library`) is still a prerequisite for most of the work in §7–§8 and is good first-issue material.
